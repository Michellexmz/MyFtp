    ...
    for i in range(4):
	    ...
	    while (t==1):
		    ...
		    sou=BeautifulSoup(torrentid_doc)
                    tag = sou.find("table",attrs={"width":"940" ,"cellspacing":"0","cellpadding":"5","border":"1" ,"align":"center"})
                    sou = BeautifulSoup(str(tag))
	      	    m=len(sou.html.body.table.findAll("tr"))
                    if m<=1:t=0
		    if m>1:
                     for y in range(m-1):  
	              a=sou.html.body.table.find_all("tr")[y+1].find_all("td")[5].string
		      #print a
		      p=0
                      if m<26:t=0
                      #print len(soup.html.body.table.findAll("tr"))
	              ...
	              if a[0]=='<':
                             shu[1]=shu[1]+1  
			     continue   
		      while (a[p]>='0')and(a[p]<='9'):
                             p=p+1
                      b=a.decode('utf8')[p:p+1].encode('utf8')
                      if (b=='秒')or(b=='分')or(b=='天')or(b=='时'): 
 			     shu[1]=shu[1]+1
			     continue
                      if b=='月':
			     if p==1:
	    ...
	    for k in range(12):torrent_file.write(str(shu[k+1])+'\n')
            time.sleep(5)
